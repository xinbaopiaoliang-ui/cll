...
next stage. Version `0.6.0` keeps the legacy TCP/UDP `ping` probe, supports
...
client integration testing, exposes probe/auth/session counters in `/health`,
and includes the optional HMAC-signed control-plane report loop. Standalone mode
leaves backend reporting disabled unless `--enable-control-plane` is passed
