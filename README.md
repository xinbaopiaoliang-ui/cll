...
next stage. Version `0.25.1` keeps the legacy TCP/UDP `ping` probe, supports
...
game-to-node target mappings without direct MySQL access. Version `0.25.1`
fixes startup schema migration compatibility on MySQL installs that return
`information_schema` marker values as signed `BIGINT`. Version `0.25.0`
