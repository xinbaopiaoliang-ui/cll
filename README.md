...
next stage. Version `0.5.0` keeps the legacy TCP/UDP `ping` probe, supports
JSON `xaccel/1` client probe responses, verifies optional `xat.v1` HMAC client
tokens, exposes probe/auth counters in `/health`, and includes the optional
HMAC-signed control-plane report loop. Standalone mode leaves backend reporting
disabled unless `--enable-control-plane` is passed with a real backend URL.
