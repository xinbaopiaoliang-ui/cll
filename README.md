...
next stage. Version `0.7.0` keeps the legacy TCP/UDP `ping` probe, supports
...
client integration testing, forwards authenticated UDP `session.data` packets to
a target address, exposes probe/auth/session/relay counters in `/health`, and
includes the optional HMAC-signed control-plane report loop. Standalone mode
leaves backend reporting disabled unless `--enable-control-plane` is passed with
a real backend URL.
