...
next stage. Version `0.27.0` keeps the legacy TCP/UDP `ping` probe, supports
...
game-to-node target mappings without direct MySQL access. Version `0.27.0`
adds an operations-page connect-intent diagnostic tool for checking the selected
candidate node, route target, transport, credential expiry, and raw scheduling
response from the browser dashboard. Version `0.26.2` moves the
