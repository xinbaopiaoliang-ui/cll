...
next stage. Version `0.25.3` keeps the legacy TCP/UDP `ping` probe, supports
...
game-to-node target mappings without direct MySQL access. Version `0.25.3`
stabilizes the Node Management detail layout by removing the sticky detail
header, aligning action forms, and making Health JSON visible in a bounded
debug panel. Version `0.25.2` refines the `/admin` header typography and status
