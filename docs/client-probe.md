...
- The selected node receives UDP probe traffic.
...
- The node validates `route_policy_hash` when the ticket contains a dynamic
  route policy.
- The node creates a short-lived session.
...
When `route_policy.targets[]` includes a concrete target for `--target-protocol`,
the probe can derive the first target automatically. Pass `--target-host` and
`--target-port` for `host_type=any`, Steam/SDR-style port checks, or a specific
client-observed IP.
