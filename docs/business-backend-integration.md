...
游戏管理和游戏路由的主入口应放在业务后台，而不是控制面板。

- 业务后台负责主数据：游戏、区服、套餐、用户、订单、权益、运营策略。
- 控制面负责执行副本：节点状态、调度、路由下发、健康检查、远程运维、操作日志。
- 控制面里的“游戏管理”后续应逐步调整为业务后台同步过来的只读快照，主要用于排查数据是否同步成功。
- 控制面里的“游戏路由”后续保留为运维兜底和联调工具，用于业务后台故障、临时救急、节点转发排查，不作为日常配置主入口。
- 客户端不直接访问控制面管理接口，业务后台完成权益判断后再调用控制面签发加速意图。

后续完善方向：业务后台完成游戏和路由的新增、编辑、批量管理；控制面只提供业务同步 API、调度结果和运维级兜底入口。
